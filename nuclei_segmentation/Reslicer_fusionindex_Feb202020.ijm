/* Reslicer
for fusion index,adapted by Jose Cadavid from file from Amir Meysami Fard. Notation is similar to other folders but this one doesn't loop through drugs or reps
it is meant to reslice stacks
Updated feb 25 2020
...
path_parent = getDirectory("Choose Input Directory containing replicates");								//Promts user to select an input folder containing the files to be processed
...
outputPath = path_grandparent + separator + parent_name + "_resliced";				//Generates the file path for the output folder based on input folder name
...
for(j=0; j<path_parent_array.length; j++) {	 // loop through replicates
	...
	//output_drug = outputPath + separator +  "Rep"+(j+1);//File.getName(path_parent_array[j]);
	...
	for (p = 0; p<path_drug_array.length; p++){ //loop through timepoints
		...
		//output_rep = output_drug + separator + "TimePoint"+(p+1);//File.getName(path_drug_array[p]);
		...
		for (q = 0; q<path_rep_array.length; q++){ //loop through images
			...
			if (endsWith(fileTitle,".oif") == 1 || endsWith(fileTitle,".oib") == 1){
				...
				//output_im = output_rep + separator + im_name;
				//File.makeDirectory(output_im);
				//Image counter
				ii=ii+1;
//Convert to 8 bit
				run("8-bit");
...
//Im3 is SAA
selectImage(3);
//saveAs("tif", output_im + separator + "SAA_reslice"+d2s(width,3)+".tif");
saveAs("tif", outputPath + separator + "SAA_reslice"+ii+".tif");
//Im4 is nuc
selectImage(4);
//saveAs("tif", output_im + separator + "Nuc_reslice"+d2s(width,3)+".tif");
